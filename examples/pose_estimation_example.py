...
def extract_pose_features(pose_data_path):
    ...
    try:
        ...
        upper_cols = [col for col in df.columns if any(part in col.lower() for part in upper_body_parts)]
        lower_cols = [col for col in df.columns if any(part in col.lower() for part in lower_body_parts)]
