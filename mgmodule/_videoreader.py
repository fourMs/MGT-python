...
def mg_videoreader(filename, starttime = 0, endtime = 0, skip = 0, contrast = 0, brightness = 0, crop = 'None'):
    ...
    if skip != 0:
        vidcap, length, fps, width, height = mg_skip_frames(of, fex, vidcap, skip, fps, width, height)
        of = of + '_skip'
    ...
    if contrast != 0 or brightness != 0:
        vidcap = mg_contrast_brightness(of, fex, vidcap,fps,length,width,height,contrast,brightness)
        of = of + '_cb'
