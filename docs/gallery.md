# Gallery

Every picture below is something the toolbox makes, rendered from the bundled example
videos. Click any image to open the page that explains the function behind it. If you
know what you want an analysis to look like but not what it is called, this page is
the index to use.

## Looking at a recording

| | | |
|---|---|---|
| [![Motion video](images/examples/motion.gif)](user-guide/video-analysis.md) | [![Motiongram](images/examples/motiongram_v.png)](user-guide/video-analysis.md) | [![Videogram](images/examples/videogram_v.png)](user-guide/video-analysis.md) |
| motion video | motiongram | videogram |
| [![Average image](images/examples/average.png)](user-guide/video-analysis.md) | [![History video](images/examples/history.gif)](user-guide/video-analysis.md) | [![Motion heatmap](images/examples/heatmap.png)](user-guide/video-analysis.md) |
| average image | history video | motion heatmap |
| [![Stroboscope](images/examples/stroboscope.png)](user-guide/video-analysis.md) | [![Silhouette waterfall](images/examples/silhouette_waterfall.png)](user-guide/video-analysis.md) | [![Motion history image](images/examples/motionhistory.png)](user-guide/video-analysis.md) |
| stroboscope | silhouette waterfall | motion history image |
| [![Contact sheet](images/examples/filmstrip.png)](musicalgestures/_contactsheet.md) | [![Multi-shot](images/examples/dancer_multishot.png)](musicalgestures/_multishot.md) | [![Eulerian magnification](images/examples/eulerian.gif)](user-guide/video-analysis.md) |
| contact sheet | multi-shot | Eulerian magnification |

## Measuring motion

| | | |
|---|---|---|
| [![Motion descriptors](images/examples/motiondescriptors.png)](user-guide/video-analysis.md) | [![Optical flow](images/examples/flow_dense.gif)](user-guide/video-analysis.md) | [![Motion vectors](images/examples/motionvectors.gif)](user-guide/video-analysis.md) |
| motion descriptors | optical flow | motion vectors |
| [![Motion tempo](images/examples/motiontempo.png)](user-guide/video-analysis.md) | [![Directograms](images/examples/directograms.png)](user-guide/video-analysis.md) | [![Self-similarity matrix](images/examples/ssm.png)](user-guide/video-analysis.md) |
| motion tempo | directograms | self-similarity matrix |

## Bodies: positions and postures

| | | |
|---|---|---|
| [![Pose landmarks](images/examples/pose_landmarks.gif)](user-guide/pose-tracking.md) | [![Pose timeline](images/examples/dancer_pose_timeline_strip.png)](user-guide/pose-tracking.md) | [![Posegram](images/examples/dancer_posegram.png)](user-guide/pose-tracking.md) |
| pose landmarks | pose timeline | posegram |
| [![Average pose](images/examples/pose_average.png)](user-guide/pose-tracking.md) | [![Centred pose](images/examples/pose_center.png)](user-guide/pose-tracking.md) | [![Skeleton timeline](images/examples/dancer_skeleton_timeline.png)](user-guide/pose-tracking.md) |
| average pose | centred pose | skeleton timeline |
| [![Effort profile](images/examples/dancer_effort_profile.png)](musicalgestures/_effort.md) | [![Pose waterfall](images/examples/pose_waterfall_trajectories.png)](user-guide/pose-tracking.md) | [![Posturography](images/examples/posturography.gif)](user-guide/sound-movement-toolkit.md) |
| effort profile | pose waterfall | postural sway |

## Sound, and sound with movement

| | | |
|---|---|---|
| [![Spectrogram](images/examples/spectrogram.png)](user-guide/audio-analysis.md) | [![Tempogram](images/examples/tempogram.png)](user-guide/audio-analysis.md) | [![Chromagram](images/examples/chromagram.png)](user-guide/audio-analysis.md) |
| spectrogram | tempogram | chromagram |
| [![Phase synchrony](images/examples/phase_synchrony.png)](user-guide/audio-video.md) | [![Tempo similarity](images/examples/tempo_similarity.png)](user-guide/audio-video.md) | [![Body-audio coupling](images/examples/body_audio_coupling.png)](user-guide/audio-video.md) |
| phase synchrony | tempo similarity | body–audio coupling |
| [![Pulse segmentation](images/examples/pulse_segmentation.gif)](user-guide/sound-movement-toolkit.md) | [![Cross-modal alignment](images/examples/alignment_xcorr.gif)](user-guide/sound-movement-toolkit.md) | [![Structure comparison](images/examples/structure_comparison.png)](user-guide/audio-video.md) |
| pulse segmentation | cross-modal alignment | structure comparison |

## Rooms, segments, and special formats

| | | |
|---|---|---|
| [![Room plate](images/examples/room_and_restless.png)](musicalgestures/_plate.md) | [![Impacts](images/examples/impacts.png)](musicalgestures/_impacts.md) | [![Timeline](images/examples/timeline_shaded.png)](musicalgestures/_timeline.md) |
| room plate | impacts | annotation timeline |
| [![Anglegram](images/examples/anglegram.png)](user-guide/video-analysis.md) | [![Audio-energy-map overlay](images/examples/aem_overlay.gif)](user-guide/video-analysis.md) | [![Grid QoM](images/examples/grid_qom.gif)](user-guide/video-analysis.md) |
| anglegram (360) | audio-energy map (360) | grid quantity of motion |
