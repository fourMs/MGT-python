...
- EAC normalisation: after projecting the ray onto the unit cube face, each in-face coordinate `t` becomes `atan(t) * 4/π` (range −1..1), then `u = (±t ± 1)/2` per the face table below.
- Face UV table (from `FindFaceUV`, with `p.x=cos(lat)sin(lon), p.y=cos(lat)cos(lon), p.z=sin(lat)`; q = p projected to the face, EAC-normalised):
